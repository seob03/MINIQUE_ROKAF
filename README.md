...
> BackEnd (FullStack 기여) - 이민섭
- `Node.js`
- `React.js` (React Hook & State 담당)
- `Express`
- `MongoDB`
- `Socket.IO`
- `Passport`<br><br><br><br>
...
- 상품마다 채팅방 생성 가능, 채팅방 많아지면 스크롤 제공
...
#### 4. 채팅방 부가 기능
- 상품 정보와 판매자 정보 통합 표시
- 좌측의 채팅방 선택을 통해 원하는 채팅방 입장
- 이전에 나눴던 채팅 메시지 로그 저장
...
#### 프로젝트 전체 파일 구조 및 네이밍의 미흡
- BE와 FE로 구분해서 프로젝트를 진행하였으나, 프로젝트를 진행하면서 파일 이동이 잦아 불편했다.(콘솔 `cd` 커맨드만 둘이서 최소 1000번은 쓴 듯 하다.)

#### React Hook에 대한 집중적인 학습 시간 부족
- 이번 역할이 백엔드였지만 의도치 않게 React Hook과 State 관리를 맡게 된 만큼 React에 대해 정말 많은 이해가 필요했지만
  React 자체가 이번 프로젝트를 하게 되면서 처음 써본 스택이었으며, 전역 전 휴가까지의 기한 내에 빠르게 프로젝트를 진행하기 위해서는 React를 집중적으로 따로 공부할 시간이 없었다.
  불필요한 랜더링을 줄이는 `useMemo` 또는 커스텀 Hook을 제작해서 사용하면 더 많은 성능 개선이 가능했을 거 같지만 프로젝트 내에서는 거의 `useEffect` 및 의존성 배열로만 훅을 작성한 거 같아서 아쉽다.
  언젠가 또 Full Stack 기여를 하는 프로젝트가 있다면, Hook에 대해 더 많은 공부를 해보는 것도 좋을 거 같다. 

#### 에러 처리의 일관성 부족
- 일부 라우트에서는 상세한 에러 메시지를 반환 BUT 특정 라우트에서는 단순히 "에러가 발생했습니다" 정도의 메시지만 반환했던 점
- 좀 더 상세한 에러 메시지를 통해 더 쉽게 디버깅을 할 수 있었을 것 같다. <br><br><br>
...
- styled-components를 사용하는 경우도, CSS 클래스 셀렉터를 사용하는 경우도 혼재되어 있어 통일성 부족<br><br><br>
...
- 커밋 메시지 컨벤션을 보기 쉽게 통일하면 더 좋았을 것으로 생각된다.
